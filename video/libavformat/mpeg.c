...
static void flush_packet(AVFormatContext *ctx,
		int stream_index,
		int last_pkt)
{
    MpegMuxContext *s = ctx->priv_data;
    ...
    put_byte(&ctx->pb, (uint16_t)((0x02 << 4) | (((timestamp >> 30) & 0x07) << 1) | 1));
                                  ...
                                  AVPacket *pkt)
{
 ...
 redo:
    ...
    if (startcode == PADDING_STREAM ||
        ...
        /* skip them */
        len = get_be16(&s->pb);
				if( get_mem_buffer_size( &s->pb )< len )
				{
					url_fseek( &s->pb, pos, SEEK_SET );
					return AVILIB_NEED_DATA;
				}
    ...
    if ((c & 0xf0) == 0x20) {
        ...
        if ((c & 0x30) != 0) {
            ...
            return AVILIB_ENCRYPTED;
        }
        flags = get_byte(&s->pb);
        header_len = get_byte(&s->pb);
				if( get_mem_buffer_size( &s->pb )< header_len )
				{
					url_fseek( &s->pb, pos, SEEK_SET );
					return AVILIB_NEED_DATA;
				}
