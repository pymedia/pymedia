...
//FIXME check what gcc generates for 64 bit on x86 and possible write a 32 bit ver of it
    if(w==2 && h==2){
        ...
        *(uint64_t*)(p + 8+1*stride)= size==4 ? val*uint64_t_C(0x0100000001) : val*uint64_t_C(0x0101010101010101);
    }else if(w==16 && h==4){
        *(uint64_t*)(p + 0+0*stride)=
        *(uint64_t*)(p + 8+0*stride)=
        *(uint64_t*)(p + 0+1*stride)=
        ...
        *(uint64_t*)(p + 8+3*stride)= size==4 ? val*uint64_t_C(0x0100000001) : val*uint64_t_C(0x0101010101010101);
    }else
        assert(0);
}
