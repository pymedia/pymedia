...
class VPlayer:
    ...
    try:
	  ...
	  if harryA == 1:
          ...
          if d[ 3 ] > 0:
             ...
             aPTS= lastAPts    
          lastAPts = aPTS
          aPTS= float( aPTS ) / 90000
          # See if we have anything to play
          if len( s ) > 0 and self.seek== 0:
            stsDelay = time.time() - self.getSTS()
	    stsDelay = aPTS-stsDelay
	    if harryA == 0:
                if stsDelay > 0:
                  time.sleep( stsDelay )
                self.snd.play( s )
  ...
  def vid( self, queue ):
    frRate= 0
    vc= surfaces= None
    stsDelay= sndDelay= lastPts= pts= vPTS= hurry= 0
    clock= 0
    delay= .0
    ...
    try:
      while 1:
	    ...
	    self.setSTS()
          ...
          #print 'PTS' , vfr.frame_pts
          if hurry == 1:
              lastPts = vfr.frame_pts
              if vfr.pict_type == 1 and vfr.frame_pts > 0:
                self.setJump(float( vfr.frame_pts ) / 90000)
                hurry = 0
              else:                
                continue
          ...
          if self.frameNum> 0:           
	    stsDelay = time.time() - self.getSTS()
	    stsDelay = vPTS-stsDelay 
            if stsDelay > 0 and hurry == 0:
              time.sleep( stsDelay )
          else:
            frRate= float(vfr.rate_base)/ float(vfr.rate)
          
          if self.overlay and vfr.data and  hurry == 0:
              if stsDelay > 0 or vfr.pict_type == 1:
  ...
  def setJump( self , ttime):
    self.StartTime =  time.time() - ttime
    self.Jump = ttime
